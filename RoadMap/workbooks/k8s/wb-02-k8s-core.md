...
Services - объект, обеспечивающий стабильный доступ к приложениям, запущенным в подах. Бывают 4 видов:
 ...
 Headless Service — это не пятый type, а комбинация type: ClusterIP + clusterIP: None. Он отключает виртуальный IP и балансировку, и DNS возвращает IP всех Pod'ов напрямую. Используется в StatefulSet и stateful-приложениях, где важна идентичность конкретного Pod'а.
